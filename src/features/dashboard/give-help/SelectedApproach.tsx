...
function SelectedApproach() {
  const { total_amount } = usePaymentProcess((state) => state?.PaymentInfo);
  const searchParams = useSearchParams();
  const router = useRouter();
  const ProcessItem = searchParams.get('process') || '';
  ...
  const handleToPay = () => {
    router.push(`/dashboard/payment/about-payment?process=${ProcessItem}`);
  };
  return (
